...
def extract_fuses(fuses, field, *, default=None):
    ...
    if default is None:
        assert (
            False
        ), f"fuses {field['fuses']}: extracted {value}, known {field['values']}"
    return default
...
def replace_fuses(fuses, field, key):
    for value_key, value in field["values"].items():
        ...
        assert (
            False
        ), f"fuses {field['fuses']}: replace with {key}, known {field['values']}"
...
class FuseTool:
    ...
    def get_user(self, index):
        ...
        value = sum(
            self.fuses[fuse] << n_fuse for n_fuse, fuse in enumerate(field["fuses"])
        )
        if self.verbose:
            self.print(
                "USR{}: ({}) {:02x} [{:5}..{:5}]".format(
    ...
    def get_device(self, filters):
        ...
        for prefix in ("GCLR", "GCLK", "GOE"):
            for switch_name, switch in self.device["globals"].items():
                if not switch_name.startswith(prefix):
                    continue
    ...
    def set_option(self, option_name, option, value):
        ...
        if value not in option["values"]:
            raise SystemExit(
                f"Option {option_name} cannot be set to '{value}'; "
        ...
        if value == "VCC":
            ...
            def apply_net(net, value):
                if net in pterm_points:
                    ...
                    raise SystemExit(
                        f"Product term cannot contain net '{net}'; "
            ...
            if all(net.startswith("+") or net.startswith("-") for net in expr):
                ...
                raise SystemExit(
                    f"An expression should either modify existing product term "
        ...
        with self.hierarchy("CFG"):
            for option_name in macrocell_options:
                ...
                if match_filters_last(filters.get("CFG", filters), (option_name,)):
                    changed += self.set_option(
                        option_name, macrocell[option_name], value
                    )
    ...
    def set_switch_mux(self, switch_name, switch, value):
        ...
        if value not in mux["values"]:
            raise SystemExit(
                f"Switch {switch_name} cannot select net '{value}'; "
    ...
    def set_switch(self, switch_name, switch, filters, value):
        ...
        for option_name in global_options:
            if option_name not in switch:
                continue
            if match_filters_last(filters, (option_name,)):
                changed += self.set_switch_option(
                    switch_name, option_name, switch[option_name], value
                )
        ...
        with self.hierarchy("CFG"):
            for config_name in config_options:
                if match_filters_last(filters.get("CFG", filters), (config_name,)):
                    changed += self.set_option(
                        config_name, self.device["config"][config_name], value
                    )
    ...
    def set_special_pin(self, pin, pin_config, filters, value):
        ...
        with self.hierarchy(pin):
            for option_name in special_pin_options:
                ...
                if match_filters_last(filters, (option_name,)):
                    changed += self.set_option(
                        option_name, pin_config[option_name], value
                    )
...
def arg_parser():
    ...
    subparsers = parser.add_subparsers(
        metavar="COMMAND", dest="command", required=True, help="Operation to perform."
    )
...
def main():
    ...
    if device_fuse_count != len(fuses):
        raise SystemExit(
            f"Device has {device_fuse_count} fuses, JED file "
