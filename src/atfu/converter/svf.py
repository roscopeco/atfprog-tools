...
_tap_stable_states = ("RESET", "IDLE", "IRPAUSE", "DRPAUSE")
...
class SVFLexer:
    ...
    _scanner = tuple(
        (re.compile(src, re.A | re.I | re.M), act)
        for src, act in (
            (r"\s+", None),
            (r"(?:!|//)([^\n]*)(?:\n|\Z)", None),
            (r"({})(?=\s+|[;()]|\Z)".format("|".join(_keywords)), lambda m: m[1]),
            (r"(\d+)(?=[^0-9\.E])", lambda m: int(m[1])),
            (r"(\d+(?:\.\d+)?(?:E[+-]?\d+)?)", lambda m: float(m[1])),
            (
                r"\(\s*([0-9A-F\s]+)\s*\)",
        ...
        while True:
            for token_re, action in self._scanner:
                ...
                raise SVFParsingError(
                    "unrecognized SVF data at line %d, column %d (%s...)"
                    % (
                        *self.line_column(),
...
class SVFParser:
    ...
    def _parse_unexpected(self, expected, valid=()):
        ...
        if valid:
            self._parse_error(
                "expected %s (one of %s), found %s"
                % (expected, ", ".join(valid), actual)
            )
        else:
            self._parse_error("expected %s, found %s" % (expected, actual))
    ...
    def _parse_scan_data(self, length):
        ...
        if value[length:].count(1) != 0:
            ...
            residue.reverse()
            self._parse_error(
                "scan data length %d exceeds command length %d" % (len(value), length)
            )
    ...
    def parse_command(self):
        ...
        if command is None:
            ...
            *path_states, stable_state = states
            ...
            if (
                self._param_mask[command] is None
                or len(self._param_mask[command]) != length
            ):
                self._param_mask[command] = bitarray(length, endian="little")
                self._param_mask[command].setall(1)
            if (
                self._param_smask[command] is None
                or len(self._param_smask[command]) != length
            ):
            ...
            while True:
                parameter = self._try(
                    self._parse_keywords, ("TDI", "TDO", "MASK", "SMASK")
                )
            ...
            if command == "HIR":
                result = self._handler.svf_hir(
                    tdi=param_tdi, smask=param_smask, tdo=param_tdo, mask=param_mask
                )
            if command == "SIR":
                result = self._handler.svf_sir(
                    tdi=param_tdi, smask=param_smask, tdo=param_tdo, mask=param_mask
                )
            if command == "TIR":
                result = self._handler.svf_tir(
                    tdi=param_tdi, smask=param_smask, tdo=param_tdo, mask=param_mask
                )
            if command == "HDR":
                result = self._handler.svf_hdr(
                    tdi=param_tdi, smask=param_smask, tdo=param_tdo, mask=param_mask
                )
            if command == "SDR":
                result = self._handler.svf_sdr(
                    tdi=param_tdi, smask=param_smask, tdo=param_tdo, mask=param_mask
                )
            if command == "TDR":
                result = self._handler.svf_tdr(
                    tdi=param_tdi, smask=param_smask, tdo=param_tdo, mask=param_mask
                )

        elif command == "RUNTEST":
            run_state = self._try(self._parse_tap_stable_state)
            run_params = self._try(
                lambda: (self._parse_value(int), self._parse_keywords(("TCK", "SCK")))
            )
            if run_params is None:
                run_count, run_clock = None, "TCK"
                min_time, _ = self._parse_value((int, float)), self._parse_keyword(
                    "SEC"
                )
            else:
                run_count, run_clock = run_params
                min_time, _ = self._try(
                    lambda: (
                        self._parse_value((int, float)),
                        self._parse_keyword("SEC"),
                    )
                ) or (None, None)
            if self._try(self._parse_keyword, "MAXIMUM"):
                max_time, _ = self._parse_value((int, float)), self._parse_keyword(
                    "SEC"
                )
            else:
                max_time = None
            ...
            result = self._handler.svf_runtest(
                run_state=run_state,
                run_count=run_count,
                run_clock=run_clock,
                min_time=min_time,
...
class SVFLexerTestCase(unittest.TestCase):
    ...
    def test_keyword(self):
        self.assertLexes("TRST", ["TRST"])
        self.assertLexes("TRST OFF;", ["TRST", "OFF", ";"])
    ...
    def test_bitarray(self):
        ...
        self.assertLexes(
            "(A\n5)", [bitarray("10100101")]
        )  # Test literals split over two lines
        self.assertLexes(
            "(A\n\t5)", [bitarray("10100101")]
...
class SVFParserTestCase(unittest.TestCase):
    ...
    def test_frequency(self):
        self.assertParses("FREQUENCY;", [("svf_frequency", {"frequency": None})])
        self.assertParses("FREQUENCY 1E6 HZ;", [("svf_frequency", {"frequency": 1e6})])
        self.assertParses(
            "FREQUENCY 1000 HZ;", [("svf_frequency", {"frequency": 1000})]
        )

        self.assertErrors("FREQUENCY 1E6;", "expected HZ")

    def test_trst(self):
        self.assertParses("TRST ON;", [("svf_trst", {"mode": "ON"})])
        self.assertParses("TRST OFF;", [("svf_trst", {"mode": "OFF"})])
        self.assertParses("TRST Z;", [("svf_trst", {"mode": "Z"})])
        self.assertParses("TRST ABSENT;", [("svf_trst", {"mode": "ABSENT"})])

        self.assertErrors("TRST HZ;", "expected TRST mode")

    def test_state(self):
        self.assertParses("STATE IDLE;", [("svf_state", {"state": "IDLE", "path": []})])
        self.assertParses(
            "STATE IRUPDATE IDLE;",
        ...
        self.assertErrors("STATE;", "at least one state required")
        self.assertErrors("STATE IRSHIFT;", "last state must be a stable state")
        self.assertErrors("STATE RESET IRSHIFT;", "last state must be a stable state")

    def test_endir_enddr(self):
        for command, event in [("ENDIR", "svf_endir"), ("ENDDR", "svf_enddr")]:
            self.assertParses(
                "{c} IRPAUSE;".format(c=command), [(event, {"state": "IRPAUSE"})]
            )

            self.assertErrors(
                "{c} IRSHIFT;".format(c=command), "expected stable TAP state"
            )
    ...
    def test_hir_sir_tir_hdr_sdr_tdr(self):
        for command, event in [
            ...
            self.assertParses(
                "{c} 8 TDI(a); {c} 8;".format(c=command),
                [
                    (
                        event,
                        {
            ...
            self.assertParses(
                "{c} 8 TDI(a) SMASK(3); {c} 8; {c} 12 TDI(b);".format(c=command),
                [
                    (
                        event,
                        {
            ...
            self.assertParses(
                "{c} 8 TDI(0) TDO(a) MASK(3); {c} 8; "
                "{c} 8 TDO(1); {c} 12 TDI(0) TDO(b);".format(c=command),
                [
                    (
                        event,
                        {
            ...
            self.assertErrors(
                "{c} 8 TDI(aaa);".format(c=command),
                "scan data length 12 exceeds command length 8",
            )
            self.assertErrors(
                "{c} 8 TDI(0) TDI(0);".format(c=command),
                "parameter TDI specified twice",
            )
            self.assertErrors(
                "{c} 8;".format(c=command), "initial value for parameter TDI required"
            )
            self.assertErrors(
                "{c} 8 TDI(aa); {c} 12;".format(c=command),
                "parameter TDI needs to be specified again because "
                "the length changed",
            )
    ...
    def test_runtest(self):
        ...
        self.assertParses(
            "RUNTEST 20000 TCK 1E3 SEC MAXIMUM 1E6 SEC;",
            [
                (
                    "svf_runtest",
                    {
        ...
        self.assertParses(
            "RUNTEST 20000 TCK 1E3 SEC MAXIMUM 1E6 SEC ENDSTATE RESET;",
            [
                (
                    "svf_runtest",
                    {
        ...
        self.assertParses(
            "RUNTEST 20000 TCK ENDSTATE RESET; RUNTEST 100 TCK;",
            [
                (
                    "svf_runtest",
                    {
        ...
        self.assertParses(
            "RUNTEST RESET 20000 TCK ENDSTATE RESET; RUNTEST IDLE 100 TCK;",
            [
                (
                    "svf_runtest",
                    {
        ...
        self.assertParses(
            "RUNTEST 200 TCK ENDSTATE RESET; RUNTEST 1 SEC;",
            [
                (
                    "svf_runtest",
                    {
        ...
        self.assertErrors("RUNTEST;", "expected number")
        self.assertErrors(
    ...
    def test_piomap(self):
        self.assertParses(
            "PIOMAP (IN FOO OUT BAR);", [("svf_piomap", {"mapping": "IN FOO OUT BAR"})]
        )

        self.assertErrors("PIOMAP;", "expected data")

    def test_pio(self):
        self.assertParses("PIO (LHZX);", [("svf_pio", {"vector": "LHZX"})])

        self.assertErrors("PIO;", "expected data")
