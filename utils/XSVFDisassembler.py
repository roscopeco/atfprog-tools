...
class XSVFDisassembler(XSVFDecoder.XSVFDecoder):
    ...
    def add_arguments(p):
        ...
        p.add_argument(
            ...
            action="store_true",
            # type=bool,
            help="Do not output bytes" " (default=%(default)s)",
        )
    ...
    def __init__(self, args):
        ...
        self._instruction_handlers = (
            ...
            self.disasm_xwait,
            self.disasm_invalid,  # TODO should probably be some other instruction...
            self.disasm_invalid,  # TODO should probably be some other instruction...
            self.disasm_invalid,  # TODO should probably be some other instruction...
            self.disasm_invalid,  # TODO should probably be some other instruction...
            self.disasm_xtrst,
    ...
    def disasm_xsir(self):
        p = (
            " {:d} {:s}".format(
                self.sirsize_bits, self.format_byte_list(self.tdi).strip()
            ),
        )
        self.pretty_disasm(p)
    ...
    def disasm_xsir2(self):
        p = (" {:d}".format(self.sirsize_bits), self.format_byte_list(self.tdi))
        self.pretty_disasm(p)
    ...
    def disasm_xtrst(self):
        # TODO not handled, just ignores arg
        p = " <ignore>"
        self.pretty_disasm(p)

    def disasm_invalid(self):
        # TODO not handled, just ignores arg
        p = " <ignore>"
        self.pretty_disasm(p)
